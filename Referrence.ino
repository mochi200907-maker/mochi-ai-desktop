...
void applyVolume(uint8_t* data,size_t len,float vol){
  ...
  const float alpha = 0.995f;   // stronger high-pass
  const float gain  = 0.65f;    // lower power to protect speaker
  ...
  for(size_t i=0;i<len/2;i++){
    ...
    // High-pass filter stage 2 (extra bass cut)
    float yb = alpha * (y2 + y - x2);
    x2 = y;
    y2 = yb;
